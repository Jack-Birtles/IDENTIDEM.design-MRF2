...
// since each context uses a different size. Defined at file scope (not in an
// anonymous namespace) because interface_config_screens.cpp also calls it.
...
// Menu helpers exposed via interface_menu_helpers.h so the config-screens
// translation unit can use them. Kept here (rather than a third .cpp) so the
// existing main-UI code that also calls them needs no change.
